...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("filename:" + args[0] + " sizeX=" + args[1]
				+ " sizeY=" + args[2] + " fractalNumber=" + fractalNumber);
		// Allocate the canvas
		...
		default:
			System.out
					.println("Incompatible fractal number entered, not in [1,6]");
			System.exit(1);
			break;
		}
		...
		if (fractal1) {
			...
			String[] symbols = new String[numSymbols]; // symbols that can be
														// used to make strings
			String[] rules = new String[numSymbols]; // rules that expand each
														// symbol
			...
			MakeFractal kochSnowFlake = new MakeFractal(symbols, numSymbols,
					axion, rules, n);
			String computedFractal = kochSnowFlake.buildFractal();
			kochSnowFlake.prettyPrint();
			DrawFractal frac1 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac1.computeLines();
		...
		if (fractal2) {
			...
			MakeFractal kochCurve = new MakeFractal(symbols, numSymbols, axion,
					rules, n);
			String computedFractal = kochCurve.buildFractal();
			kochCurve.prettyPrint();
			DrawFractal frac2 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac2.computeLines();
		...
		if (fractal3) {
			...
			MakeFractal serpinskiTriangle = new MakeFractal(symbols,
					numSymbols, axion, rules, n);
			String computedFractal = serpinskiTriangle.buildFractal();
			serpinskiTriangle.prettyPrint();
			DrawFractal frac3 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac3.computeLines();
		...
		if (fractal4) {
			...
			MakeFractal dragonCurve = new MakeFractal(symbols, numSymbols,
					axion, rules, n);
			String computedFractal = dragonCurve.buildFractal();
			dragonCurve.prettyPrint();
			DrawFractal frac4 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac4.computeLines();
		...
		if (fractal5) {
			...
			MakeFractal fractalPlant = new MakeFractal(symbols, numSymbols,
					axion, rules, n);
			String computedFractal = fractalPlant.buildFractal();
			fractalPlant.prettyPrint();
			DrawFractal frac5 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac5.computeLines();
		...
		if (fractal6) {
			...
			System.out.println("length of symbols:" + symbols.length);
			symbols[0] = new String("F"); // symbols in alphabet
			rules[0] = new String("FF-[-F+F+F]+[+F-F-F]"); // One rule per
															// symbol
			String axion = "------F";
			Double scalingFactor = 1.0;

			MakeFractal fractalBush = new MakeFractal(symbols, numSymbols,
					axion, rules, n);
			String computedFractal = fractalBush.buildFractal();
			fractalBush.prettyPrint();
			DrawFractal frac6 = new DrawFractal(computedFractal, scalingFactor,
					angle);
			frac6.computeLines();
